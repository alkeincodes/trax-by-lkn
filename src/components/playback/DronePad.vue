...
function handleVolumeChange(event: Event) {
  ...
  dronePadStore.setVolume(newVolume)
}
</script>

<template>
  <div class="flex w-full gap-4 rounded-lg h-full">
    <!-- Main Controls -->
    <div class="flex flex-col flex-1 gap-4">
        ...
        <div class="flex-1">
          <DropdownMenuRoot>
            <DropdownMenuTrigger as-child>
              <button
                class="w-full flex gap-1 flex-col items-start rounded-md border border-border bg-background px-3 py-2 text-left hover:bg-accent hover:text-accent-foreground focus:border-transparent focus:outline-none focus:ring-1 focus:ring-transparent transition-colors"
              >
      ...
      <div class="grid grid-cols-4 gap-2">
        <button
          ...
          :disabled="!key"
          :class="cn(
            'h-18 rounded-md text-lg font-semibold transition-all duration-200',
            !key && 'invisible',
    ...
    <!-- Volume Control (StemRow style) -->
    <div class="flex items-center gap-3 rounded-lg border border-border bg-inner-panel p-3">
      ...
      <div class="h-full text-center flex flex-col justify-center">
        ...
        <div class="mixer-fader">
          <input
              ...
              max="100"
              class="drone"
              :value="dronePadStore.volume * 100"
        ...
        <!-- Volume Display -->
        <div class="mx-auto text-right text-xs text-muted-foreground mt-4">
          {{ Math.round(dronePadStore.volume * 100) }}%
        </div>
      </div>
    </div>
  </div>
